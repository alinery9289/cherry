...
from cherry.util.MyExceptions import (DownloadError, UploadError, FFmpegExecuteError, GPACExecuteError, InternalError)
...
'''
base module is the base class providing config handler, logger handler
'''
class base_module(object):
    def __init__(self):
        ...
        if (self.transfer_method == 'redis'):
            ...
            # redis % database   
            self.is_exist = self.redis_is_exist
...
class base_filter(base_module, Singleton):
    ...
    def __parameter_decode(self, to_filter_para_in_str):
        ...
        return filter_parameter, data_key
    ...
    def do_process_main(self, to_filter_para_in_str):
        with MyRoam.temporary_env(self.roam_path):
            ...
            filter_parameter, src = self.__parameter_decode(to_filter_para_in_str)
            ...
            self.filter_foo(filter_parameter)
...
class uploader(base_module, Singleton):
    ...
    def upload(self, to_uploader_para_in_str):
        ...
        # return_key = process_task_ID + '.' + to_uploader_para_in_json['segment_file_name'] + '.return'
        file_name = self.roam_path + '\\' + process_task_ID + '\\before\\' + to_uploader_para_in_json['segment_file_name']
        with open(file_name,'rb') as f:
            data = f.read()
            self.dump_to_storage(data_key, data)
...
class merger(base_module):
    ...
    def merge(self, *args, **kargs):
        ...
        with MyRoam.temporary_env(root_path = roam_path, director_given_name = task_ID, generate_hash_dir = False, del_roam_data = False) as roam_context: 
